...
def test_unregistered_face_is_unknown(client, monkeypatch):
    ...
    # Mock recognize_classroom_image to report 1 unknown face
    def mock_recognize(image_path):
        return {
            "recognized_students": [],
            "unknown_faces": [{"bbox": [0, 0, 10, 10]}],
            "face_count": 1
        }

    monkeypatch.setattr("app.main.recognize_classroom_image", mock_recognize)
...
# -> should dedupe to one PRESENT record.
# ------------------------------------------------------------------
def test_duplicate_detection_same_student_dedupes(client, monkeypatch):
    ...
    # Mock recognize_classroom_image returning one recognized student
    def mock_recognize(image_path):
        return {
            "recognized_students": [{"registration_number": "21CS003", "confidence": 0.9}],
            "unknown_faces": [],
            "face_count": 2
        }
    ...
    assert response.status_code == 200
    body = response.json()
    assert body["present_count"] == 1  # Deduped to 1
...
def test_empty_file_rejected(client, monkeypatch):
    monkeypatch.setattr("app.main.recognize_classroom_image", lambda path: {"recognized_students": [], "unknown_faces": [], "face_count": 0})
    response = client.post(
        "/recognize-and-mark/",
        data={"class_name": "CS101"},
